...
class Resampler(nn.Module):
    ...
    def __init__(
            self, coord_dim: int = 1, mode: str = "bilinear", padding_mode: str = "zeros"
    ):
        super().__init__()
        self.mode = mode
    ...
    def forward(self, input, coord_grid):
        ...
        # coord_grid = torch.transpose(coord_grid, self.coord_dim, -1)
        if len(im_shape) == 2:
            coord_grid = coord_grid.permute(0, 2, 3, 1).contiguous()
        else:
            coord_grid = coord_grid.permute(0, 2, 3, 4, 1).contiguous()
...
def batch_transform_efficient(
    ...
    coord_grid = torch.transpose(coord_grid, coord_dim, -1)
    # print(coord_grid.shape)
    shape = coord_grid.shape
    coord_grid = coord_grid.contiguous().view(shape[0], -1, shape[-1])  # flatten
    ...
    coord_grid = torch.transpose(coord_grid, -1, coord_dim)
